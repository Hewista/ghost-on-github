...
Options:
  ...
  --domain=<local-address>  Address of local ghost installation [default: localhost:4000].
...
def main():
    ...
    if arguments['generate']:
        ...
        def trans_local_domain_to_github_pages(text):
            #modified_text = text.replace('localhost:4000', github_url)
            modified_text = re.sub(r"localhost:2368", "yourdomain.com", text)
            modified_text = re.sub(r"127.0.0.1:2368", "yourdomain.com", text)
            return modified_text

        def fix_font_tags(text):
            modified_text = re.sub(r"http://fonts.googleapis.com",
                                   "https://fonts.googleapis.com", text)
            return modified_text

        def fix_gravatar_tags(text):
            modified_text = re.sub(r"http://www.gravatar.com/avatar/",
                                   "https://www.gravatar.com/avatar/", text)
            return modified_text
